...
function getLatestVersion()
{
    ...
    $url = 'https://license.siswift.com/updates/version/' . systemDetails()['name'];
...
function getTemplates()
{
    $param['purchasecode'] = env("PURCHASECODE");
    $param['website'] = @$_SERVER['HTTP_HOST'] . @$_SERVER['REQUEST_URI'] . ' - ' . env("APP_URL");
    $url = 'https://license.siswift.com/updates/templates/' . systemDetails()['name'];
    $result = curlPostContent($url, $param);
    if ($result) {
        return $result;
    } else {
...
{
    $fileManager = new \App\Lib\FileManager($file);
...
{
    return new \App\Lib\FileManager();
}
...
function featureValue($feature){
    ...
    $plan = Plan::query()->where('id', $plan_name)->first();
    $plan = $plan->features()->where('name->en', $feature)->first();
    $data = $plan->value;
    return $data;
