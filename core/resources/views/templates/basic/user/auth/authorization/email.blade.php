                ...
                <div class="section-header left-style">
                    <h3 class="title">{{ __(@$authorize_content->data_values->title) }}</h3>
                        <p>{{ __(@$authorize_content->data_values->description) }}</p>
                            ...
                            <form action="{{route('user.verify.email')}}" method="POST" class="contact-form mb-30-none">
                                @csrf
                                ...
                                <div class="contact-group justify-content-end">
                                    <p>@lang('Please check including your junk/spam folder. If not found, try to') <a href="{{route('user.send.verify.code')}}?type=email" class="forget-pass"> @lang('Resend code')</a></p>
